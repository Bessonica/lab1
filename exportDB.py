...
# 
# mySQL     dbmySQLorm
# SQLight   dbBook   dbMagazine


#     ORM end


#def insertSQLightIntoMysql(table1, table2, mySQlCur, mySQLdb, dbmySQLORM, tableBook, tableMagazine, tableBookMySQL,tableMagazineMySQL):

#def insertSQLightIntoMysql(table1, table2, mySQlCur, mySQLdb):
def insertSQLightIntoMysql(table1, table2, mySQlCur, mySQLdb, tableBook, tableMagazine, tableBookMySQL,tableMagazineMySQL):


    print(" TABLE BOOK ")
    print(tableBookMySQL.get())


    #   delete all data from precious activation
    for rows in (tableBookMySQL.select().dicts()):
        tableBookMySQL.get().delete_instance()

    for rows in (tableMagazineMySQL.select().dicts()):
        tableMagazineMySQL.get().delete_instance()




#              !!!!      ORM   !!!!!
    #print(rows['id'])   !!! how address
    print("all rows in books")
    for rows in (tableBook.select().dicts()):
        print(rows)
    print("all rows in magazines")
    for rows in (tableMagazine.select().dicts()):
        print(rows)

#  из tableBook  tableMagazine   в  dbmySQLORM
    for rows in (tableBook.select().dicts()):
        tableBookMySQL.create(name = rows['name'], genre = rows['genre'], year = rows['year'])
        
    for rows in (tableMagazine.select().dicts()):
        tableMagazineMySQL.create(title = rows['title'], issue = rows['issue'], issueTitle = rows['issueTitle'])
...
def insertMysqlToPostgreSQL(mySQLDB, postSQLcur, postSQLcon, mySQLtableBook, mySQLtableMagazine,pSQLtableBook, pSQLtableMagazine):
    # !!!! clean postgreSQL
    ...
    # NOW DO SAME FOR POSTRGELSQL


    # delete all data from pSQL
    for rows in (pSQLtableBook.select().dicts()):
        pSQLtableBook.get().delete_instance()
    
    for rows in (pSQLtableMagazine.select().dicts()):
        pSQLtableMagazine.get().delete_instance()



    print("all rows in books")
    for rows in (mySQLtableBook.select().dicts()):
        print(rows)
    print("all rows in magazines")
    for rows in (mySQLtableMagazine.select().dicts()):
        print(rows)


    for rows in (mySQLtableBook.select().dicts()):
        pSQLtableBook.create(name = rows['name'], genre = rows['genre'])
        
    for rows in (mySQLtableMagazine.select().dicts()):
        pSQLtableMagazine.create(title = rows['title'], issue = rows['issue'])
...
# insertSQLightIntoMysql(dbSQLightBooks, dbSQLightMagazines, mySQLcur, mySQLcon, Book, Magazine, Book1, Magazine1)
